	...
	public function finalSubmit($CAssessmentId = NULL) {
		if($CAssessmentId) {
			...
			if($res)
			{
				...
				$rsacle = $this->db->query('INSERT INTO tblratingscalewiseksa (CAssessmentId, RatingScaleId, NoOfKSA, PercentOfKSA) SELECT '.$CAssessmentId.' AS CAssessmentId, rs.RatingScaleId, COUNT(cank.CAssessmentId) AS NoOfKSA, round((100*COUNT(cank.CAssessmentId))/(SELECT COUNT(CAssessmentId) FROM tblcandidateksa WHERE CAssessmentId = 9), 2) AS PercentOfKSA FROM tblcandidateksa AS cank LEFT JOIN tblmstksa AS k ON k.KSAId = cank.KSAId LEFT JOIN tblmstratingscale AS rs ON rs.RatingScaleId = cank.RatingScaleId WHERE cank.CAssessmentId = '.$CAssessmentId.' GROUP BY rs.RatingScaleId');
				return true;
			}
			else
			{
